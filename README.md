...
## 8. 회고
#### 김동민
 - 처음으로 임하는 PM의 역할에 걱정이 앞섰지만, 비교적 두루뭉술한 일정과 역할의 분배에도 각자의 위치에서 열의와 책임감을 가지고 임한 팀원들 덕에 그러한 걱정이 무색하게 짧은 여정을 성공적으로 마무리 지을 수 있었습니다. 
 - 또한 이 프로젝트의 프론트/백엔드를 전담하며 새로이 React와 FastAPI를 학습하고 사용하게 되었는데, 구현만 신경쓰면 되던 기존의 Streamlit에서 벗어나 데이터의 통신 절차와 예외처리, 전/후처리의 중요성을 좀 더 체득할 기회가 되었습니다. React의 컴포넌트간 프롭스 교환, use후크 사용 등으로 전역 상태를 유기적으로 관리하고, FastAPI를 통해 서버 통신 규약과 대용량 데이터의 사전 준비 작업을 통해 속도를 향상시키는 경험은 웹을 넘어 프로그래밍 감각 전반에 대한 큰 향상의 기회가 되었습니다.
 -  또한 React와 THREE JS를 결합하여 사용자에게 직감적으로 와닿는 UI를 구현하고자 노력하였고, 웹 환경에서의 최적화 문제에 많은 고민을 할 수 있었습니다. 부족하긴 하지만 추후에 더 나은 모델을 만들 좋은 기회가 되었다고 생각합니다.
