...
## 8. 회고
#### 김동민
 - 처음으로 임하는 PM 역할에 걱정이 앞섰지만, 다소 유연한 일정과 역할 분배 속에서도 각자의 위치에서 열의와 책임감을 다해준 팀원들 덕분에 짧은 여정을 성공적으로 마무리할 수 있었습니다.
기술적으로는 이번 프로젝트의 프론트엔드와 백엔드를 전담하며 React와 FastAPI를 새롭게 학습하고 도입하는 도전을 했습니다. 화면 구현에 집중하던 기존의 Streamlit 환경에서 벗어나, 클라이언트-서버 간의 데이터 통신 절차, 예외 처리, 그리고 데이터 전/후처리의 중요성을 깊이 체득할 수 있었습니다. React의 Props 전달과 Hook을 활용해 상태(State)를 유기적으로 관리하고, FastAPI를 통해 대용량 데이터의 사전 처리와 통신 속도를 최적화한 경험은 웹 개발을 넘어 프로그래밍 감각 전반을 한 단계 끌어올리는 계기가 되었습니다.
또한, React와 Three.js를 결합해 사용자에게 직관적으로 와닿는 3D UI를 구현하며, 웹 환경에서의 렌더링 최적화 문제도 깊이 고민해 볼 수 있었습니다. 아직 부족한 점도 있겠지만, 이번의 치열했던 경험은 추후 더 나은 서비스를 설계하고 개발하는 데 있어 훌륭한 자양분이 될 것이라 확신합니다.
...
#### 전서연
 - 선거 기간에 진행하는 선거 결과 예측이라는 흥미로운 주제가 욕심이 났지만, 처음에는 선거 데이터랑 설문조사 결과를 합쳐서 기권율을 예측한다는 게 감이 잘 오지 않아 막막했습니다. 하지만 팀원들과 소통하면서 복잡했던 데이터 흐름을 하나씩 정리할 수 있었습니다. 전처리 팀이 깔끔하게 만들어준 데이터로 모델을 돌려봤을 때, 단순한 숫자들이 유권자들의 행동을 설명하는 가치 있는 인사이트로 변하는 게 신기하고 재미있었습니다. 또, 수치로 나온 결과를 다양한 시각화로 표현하는 과정도 데이터를 바라보는 시각을 넓힐 수 있게 된 것 같아 의미있었습니다. 잘 이끌어주신 PM님과 적극적인 팀원들과 함께여서 두 번째 단위 프로젝트도 무사히 끝낼 수 있었던 것 같습니다. 데이터 분석의 매력도 알게 되고, 좋은 팀원들을 만나서 많이 배운 프로젝트였습니다!😊
